...
const Explanation8: React.FC = () => {
  ...
  useEffect(() => {
    const fetchExplanation = async () => {
      const docRef = doc(db, "user", "DPodpzPcBj7E0u5wkXAj");
      const docSnap = await getDoc(docRef);

      if (docSnap.exists()) {
        const data = docSnap.data();
        setExplanation([{ Explanation: data.解説文8の1 }]);
        setExplanation2([{ Explanation: data.解説文8の2 }]);
        setTitle(data.タイトル8); // タイトルを取得して保存
      } else {
